const { Router, Request, Response, NextFunction, request } = require("express");
...
const { ListSettings } = require("../../../models");
...
class ListingController {
  ...
  postListing = async (req, res, next) => {
  ...
  getListing = async (req, res, next) => {
    ...
    const include = {
      include: [
        { model, as: "listingData" },
        { model, as: "amenities" },
        { model, as: "rules" },
        { model, as: "features" },
        { model, as: "tags" },
        { model, as: "photos" },
        { model, as: "exceptionDates" },
        {
          model,
          as: "accessDays",
          include: [
  ...
  getListingRules = async (req, res, next) => {
    const listingId = (req.params.id);
    if (!listingId) {
      throw new HttpException(400, `Listing ID must be provided.`);
    }
  ...
  getListingTags = async (req, res, next) => {
    const listingId = (req.params.id);
    if (!listingId) {
      throw new HttpException(400, `Listing ID must be provided.`);
    }
  ...
  getListingAccessHours = async (
    ...
    const accessDayId = (req.params.accessDayId);
    if (!accessDayId) {
