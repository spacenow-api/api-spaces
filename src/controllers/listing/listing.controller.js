...
const { _getCategories } = require("./../categories/category.controller");
...
class ListingController {
  ...
  cache = new NodeCache({ stdTTL: 28800 });
  ...
  createDraftListing = async (
    ...
    try {
      ...
      const listingObj = await Listing.create({
  ...
  claimListing = async (req, res, next) => {
  ...
  publishListing = async (req, res, next) => {
    ...
    try {
      ...
      const isToPublished = /true/i.test(req.params.status);
      if (isToPublished) {
  ...
  updateListing = async (req, res, next) => {
    ...
    try {
      ...
      let accessDaysToValidate = data.listingAccessDays;
      ...
      const isReady = await this.isReadyCheck(
        data.listingId,
        data.title,
        data.bookingType,
        data.basePrice,
      ...
      let isPublished = listingObj.isPublished;
      ...
      if (data.listingAccessDays) {
        const listingAccessDays = data.listingAccessDays;
  ...
  fetchAccessTypes = async (
    ...
    try {
      const result = await ListSettings.findAll({
        where: { typeId: 113 },
      });
      res.send(result);
    } catch (err) {
  ...
  getListingDataById = async (
    ...
    try {
      const listingDataObj = await ListingData.findOne({
  ...
  onlyOwner(req, listingObj) {
    const loggedUser = req.userIdDecoded;
    const loggedUserRole = req.userRoleDecoded;
    if (
      !loggedUser ||
  ...
  onlyOwnerOrAdmin(req, listingObj) {
    const loggedUserId = req.userIdDecoded;
    const loggedUserRole = req.userRoleDecoded;
  ...
  async fillDefaultTimeTable(accessDays) {
  ...
  isOpenForWork(listingAccessDays) {
    console.log("ACCESS DAYS ====>>>", listingAccessDays);
    ...
    const aWrongPeriod = listingAccessDays.listingAccessHours.filter(
      (o) => {
        return (
