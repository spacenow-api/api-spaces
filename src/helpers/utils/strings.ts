 ...
 */
export function escapeUnicode(str: string, shouldEscapePrintable: string) {
...
export function rtrim(str: string, chars: string[]) {
  const _chars = chars || WHITE_SPACES;

  var end = str.length - 1,
    charLen = _chars.length,
    found = true,
    i,
    c;
...
export function trim(str: string, chars: string[]) {
  const _chars = chars || WHITE_SPACES;
  return ltrim(rtrim(str, _chars), _chars);
}
